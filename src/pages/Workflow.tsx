          ...
          {STAGES.map((stage, i) => {
            ...
            return (
              <div key={stage} className="flex items-stretch gap-2">
                <div className="w-44 panel p-3 bg-surface-2">
                  <div className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold">Etapa {i+1}</div>
                  ...
                  <div className="mt-2 flex items-baseline gap-1.5">
                    <div className="text-xl font-bold tnum">{data.count}</div>
                    <div className="text-[10px] text-muted-foreground">no estágio</div>
                  </div>
                  {(data.atRisk > 0 || data.breached > 0) && (
                    <div className="mt-1 text-[10px]">
                      {data.breached > 0 && <span className="text-destructive font-semibold">{data.breached} estourado · </span>}
                      {data.atRisk > 0 && <span className="text-warning font-semibold">{data.atRisk} risco</span>}
                    </div>
                  )}
                </div>
                {i < STAGES.length - 1 && (
          ...
          return (
            <div key={stage} className="panel p-4">
              <div className="flex items-start justify-between mb-2">
                ...
                <span className="chip bg-surface-2 text-muted-foreground border-border">handoff</span>
              </div>
              <div className="grid grid-cols-2 gap-3 mt-3 text-[11px]">
                <div>
                  ...
                  <ul className="space-y-0.5">{d.inputs.map(x => <li key={x} className="flex items-start gap-1"><span className="text-primary">·</span>{x}</li>)}</ul>
                </div>
                <div>
                  <div className="text-muted-foreground uppercase tracking-wider mb-1 text-[10px]">Saídas</div>
                  <ul className="space-y-0.5">{d.outputs.map(x => <li key={x} className="flex items-start gap-1"><CheckCircle2 className="h-3 w-3 text-success shrink-0 mt-0.5" />{x}</li>)}</ul>
                </div>
              </div>
              <div className="mt-3 text-[11px]">
                <div className="text-muted-foreground uppercase tracking-wider mb-1 text-[10px]">Ferramentas</div>
                <div className="flex flex-wrap gap-1">{d.tools.map(t => <span key={t} className="chip bg-surface-2 text-muted-foreground border-border text-[10px]">{t}</span>)}</div>
              </div>
              <div className="mt-3 p-2 rounded bg-warning/5 border border-warning/20 text-[11px] flex items-start gap-2">
