...
import { AlertTriangle, MessageSquare, Phone, Mail, Camera, CheckCircle2, Clock, FileText } from "lucide-react";
...
export default function Occurrences() {
  ...
  return (
    <div className="p-6 space-y-5">
      <PageHeader
        title="Monitoring & occurrences"
        subtitle="Operational exception management · resolution & communication tracking"
        actions={<><Filters items={["All","Refusal","Delay","Damage","Address","Re-delivery","Equipment"]}/><Btn variant="primary"><AlertTriangle className="h-3 w-3"/> New occurrence</Btn></>}
      />

      <div className="grid grid-cols-4 gap-3">
        {[
          { l: "Open", v: 23, t: "destructive" },
          { l: "In review", v: 11, t: "warning" },
          { l: "Avg resolution", v: "2h 14m", t: "info" },
          { l: "Resolved (24h)", v: 184, t: "success" },
        ].map(s => (
      ...
      <div className="grid grid-cols-12 gap-4">
        <div className="col-span-12 xl:col-span-5 panel">
          <div className="panel-header">
            <div className="text-sm font-semibold">Open occurrences</div>
        ...
        <div className="col-span-12 xl:col-span-7 panel">
          <div className="panel-header">
            ...
            <div className="flex gap-1.5">
              <Btn variant="outline">Reassign</Btn>
              <Btn variant="outline">Schedule re-delivery</Btn>
              <Btn variant="primary"><CheckCircle2 className="h-3 w-3"/> Resolve</Btn>
            </div>
          </div>
          <div className="p-5 grid grid-cols-2 gap-4">
            {[
              ["Severity", sel.severity], ["Status", sel.status], ["Owner", sel.owner], ["Opened", sel.opened],
              ["Reason", sel.reason], ["Linked order", sel.order],
            ].map(([k,v]) => (
          ...
          <div className="px-5 pb-5">
            <div className="text-[10px] uppercase tracking-wider text-muted-foreground mb-2">Communication log</div>
            <div className="space-y-2">
              {[
                { ch: Phone, who: "Mon. Squad A → Driver", text: "Confirmou tentativa às 14:12. Cliente ausente.", time: "12 min" },
                { ch: MessageSquare, who: "WhatsApp → Cliente", text: "Tentativa de contato — sem resposta.", time: "9 min" },
                { ch: Mail, who: "Email → Solicitante (Vivo)", text: "Notificação de re-tentativa enviada para amanhã 09h00.", time: "5 min" },
                { ch: Camera, who: "Driver upload", text: "Foto da fachada anexada como evidência.", time: "3 min" },
              ].map((m, i) => (
                <div key={i} className="flex gap-2.5 p-2.5 rounded-md border border-border bg-surface-1">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="ml-auto text-[10px] text-muted-foreground"><Clock className="h-2.5 w-2.5 inline"/> {m.time}</span>
                    </div>
            ...
            <div className="mt-3 flex gap-2">
              <input className="flex-1 bg-surface-2 border border-border rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-primary" placeholder="Add internal note or send message…"/>
