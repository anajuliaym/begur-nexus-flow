  ...
  return (
    <div className="p-6 space-y-5">
      <PageHeader
        ...
        subtitle="Warehouse operations · separation, equipment assignment, dispatch readiness"
        actions={<><Filters items={["Wave 1","Wave 2","Wave 3","All"]}/><Btn variant="primary"><ScanLine className="h-3 w-3"/> Scan serial</Btn></>}
      />

      <div className="grid grid-cols-4 gap-3">
        {[
          { l: "In separation", v: 142, i: Layers, t: "primary" },
          { l: "Equipment ready", v: 89, i: Box, t: "info" },
          { l: "Loaded on dock", v: 22, i: Truck, t: "warning" },
          { l: "Dispatched today", v: 318, i: CheckCircle2, t: "success" },
        ].map(s => (
          <div key={s.l} className="panel p-4">
            ...
            <div className="text-[10px] text-muted-foreground mt-1">Wave 2 · cutoff 14:00</div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-12 gap-4">
        ...
        <div className="col-span-12 xl:col-span-8 grid grid-cols-3 gap-3">
          {[
            { name: "Awaiting separation", color: "warning", items: queue.slice(0, 4) },
            { name: "Picking in progress", color: "info", items: queue.slice(4, 8) },
            { name: "Ready for dispatch", color: "success", items: queue.slice(8, 12) },
          ].map(col => (
                ...
                {col.items.map(o => (
                  <div key={o.id} className="rounded-md border border-border bg-surface-1 p-2.5 hover:bg-surface-2 cursor-grab">
                    ...
                    <div className="mt-1.5 flex items-center gap-1">
                      <span className="chip text-[9px] bg-surface-3 border-border text-muted-foreground"><Package className="h-2.5 w-2.5"/>1 unit</span>
        ...
        <div className="col-span-12 xl:col-span-4 panel">
          <div className="panel-header">
            <div className="text-sm font-semibold flex items-center gap-2"><Boxes className="h-3.5 w-3.5"/> Equipment availability</div>
            <Btn variant="ghost">Inventory</Btn>
          </div>
              ...
              return (
                <div key={s.sku}>
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <span className={`tnum ${s.low ? "text-warning" : "text-muted-foreground"}`}>{free} free</span>
                  </div>
                  ...
                  <div className="text-[10px] text-muted-foreground mt-0.5">{s.allocated} allocated · {s.on} on hand</div>
                </div>
              );
            })}
          </div>
      ...
      <div className="panel">
        <div className="panel-header">
          <div className="text-sm font-semibold flex items-center gap-2"><Truck className="h-3.5 w-3.5"/> Dock workflow · today</div>
        </div>
        <div className="p-5">
          <div className="flex items-center gap-2">
            {["Receiving","Sorting","Picking","Verifying","Loading","Dispatch"].map((s, i, arr) => (
              <div key={s} className="flex-1 flex items-center gap-2">
                <div className="flex-1">
                  ...
                  <div className="text-[11px] text-muted-foreground tnum">{[88,72,142,55,22,318][i]} units</div>
